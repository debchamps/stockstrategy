...
function execute() {
  ...
  setTimeout(function() {
    strategyDao.getHighVarianceStocks(day, function(response) {
...
function executeStock(res) {
  //Place a buy order.
  var buyPrice = stockHelper.exchangeRoundOff(res.high * (1 + priceDiff / 100));
  var buyStopLoss = stockHelper.exchangeRoundOff(buyPrice * (1 - loss / 100));

  //place a sell order.

  var buyQuantity = Math.floor(WORTH / buyPrice);
  var targetBuyProfit = stockHelper.exchangeRoundOff((profit / 100) * buyPrice);
  ...
  if (TXN_EANBLED) {
    stockHelper.placeStopLossMarketOrder(
      "IMPORTANT MorningFirstBreakout NSE",
      res.symbol,
      buyQuantity,
      "BUY",
      "MIS",
  ...
  /*
      ...
      */
  var sellPrice = stockHelper.exchangeRoundOff(res.low * (1 - priceDiff / 100));
  var sellStopLoss = stockHelper.exchangeRoundOff(sellPrice * (1 + loss / 100));
